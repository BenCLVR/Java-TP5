...
public class BankTransfert extends JPanel {
	...
	public BankTransfert(JFrame frame ,final Auth cont) {
		...
		choiceAccountTransfert.addItem(BankTradesType.ADD);
		choiceAccountTransfert.addItem(BankTradesType.REMOVE);
            ...
            public void actionPerformed(ActionEvent e) { 
                 try { 	 
                	 if (choiceAccountCombo.getSelectedItem().equals(cont.currentUser.bankAccounts.get(0).getName())) {
                		 contOp.createOperation(cont.currentUser.id, cont.currentUser.getBankAccounts().get(0).getId(), Double.parseDouble(amountTransactionTextField.getText()), (BankTradesType) choiceAccountTransfert.getSelectedItem());
     					JOptionPane.showMessageDialog(null, "Operation succesfull");
     					cont.refreshBankAccount();
                	 }
                	 else {
                		 contOp.createOperation(cont.currentUser.id, cont.currentUser.getBankAccounts().get(1).getId(), Double.parseDouble(amountTransactionTextField.getText()), (BankTradesType) choiceAccountTransfert.getSelectedItem());
      					JOptionPane.showMessageDialog(null, "Operation succesfull");
      					cont.refreshBankAccount();
                	 }
